...
const addChecklistTagItem = async (templateId: string, checklistTagId: string, title: string) => {
  const res = await api.post(`${mainUrl}/template/${templateId}/checklist`, {title, tag: checklistTagId});
  return res.data;
}

const deleteChecklistItem = async (templateId: string, checklistTagId: string) => {
  const res = await api.delete(`${mainUrl}/template/${templateId}/checklist/${checklistTagId}`)
  return res.data;
}

const deleteChecklistTag = async (templateId: string, checklistTagId: string) => {
  const res = await api.delete(`${mainUrl}/template/${templateId}/tag/${checklistTagId}`)
  return res.data;
}
...
const updateChecklistItem = async (templateId: string, checklistId: string, body: {title: string}) => {
  const res = await api.put(`${mainUrl}/template/${templateId}/checklist/${checklistId}`, body)
  return res.data;
}
...
const moveChecklist = async ({templateId, body} :{templateId: string, body: ChecklistTag}) => {
  ...
  const res = await api.post(`${mainUrl}/template/${templateId}/checklist/move`, newBody)
  return res.data;
}
...
const updateProject = async (projectId: string,body: {target_ip: string[] | string,target_url: string[] | string, progress:string, name:string,description:string, }) => {
  body = {
    ...body,
...
const updateTemplate = async (templateId: string,body: {target_ip: string[] | string,target_url: string[] | string, progress:string, name:string,description:string, }) => {
  body = {
    ...body,
    target_ip: body.target_ip === '' ? [] : body.target_ip,
    target_url: body.target_url === '' ? [] :body.target_url ,
  }
  const res = await api.put(`http://localhost:3000/template/${templateId}`, body);
